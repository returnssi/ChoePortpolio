...
<body>
	...
	<c:if test="${session.m_id == null}">
		<h2 id="boardSubject">로그인을 하시면 정상적으로 볼 수 있습니다.</h2>
	</c:if>
	<c:if test="${session.m_id != null}">
	<div id="writeForm">
	<form role = "form" action="/qna/modify" method="post" enctype="multipart/form-data">
	<input type="text" name="qna_id" id="qna_id" value="${modify.qna_id}" readonly>
	<input type="text" name="qna_title" id="qna_title" placeholder="제목을 입력하세요." value="${modify.qna_title}">
	...
	<div><textarea id="qna_content" name="qna_content" placeholder="내용을 입력하세요." cols="30" rows="10">${modify.qna_content}</textarea></div>
